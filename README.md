...
社会シミュレーターを、無料枠のクラウド (Kaggle / Colab) で効率よく回すための runner 基盤。

**シミュレーターの中身は作らない。** 作るのは「手元のシミュレーターを、無料 GPU のある場所へ
...
前提: Python 3.10 以上 (標準ライブラリのみで動く。テスト実行だけ `pip install pytest`)
...
- 課金が発生しうる操作 (有料 GPU 等) はこの repo の責務外。無料枠のみ
